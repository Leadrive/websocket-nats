...
Client.prototype.createConnection = function() {
  ...
  this.selectServer();
  // Create the stream.
  this.stream = this.netCreateConnection(this.url);
  ...
  while (!client.closed && client.inbound && client.inbound.length > 0) {
	...
	if ((Date.now() - start) > client.options.yieldTime) {
	  ...
	  // setImmediate(client.processInbound.bind(this));
		setTimeout(() => {
			client.processInbound.bind(self)
		}, 10);
	  return;
	}
      }
...
util.inherits(WebSocketProxy, events.EventEmitter);
...
Client.prototype.netCreateConnection = function(url) {
